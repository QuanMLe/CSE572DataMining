...
% Gets the data from the SVD folder of phase II
...
for i = 1 : size(userNums, 1)
    ...
    forkSVDEatData = [forkSVDEatData;Fork_Eat_Data];

    userForkEat = userData(contains(userData,'Fork_NotEat'));
    load(strcat(dataPath,'\',userForkEat{1}));
    forkSVDNotEatData = [forkSVDNotEatData;Fork_NotEat_Data];
...
for i = 1 : size(forkSVDEatData, 1)
    ori_z_eat = forkSVDEatData(:,i);
    ori_z_not_eat = forkSVDNotEatData(:,i);
...
% Fork SVD eating data 
% T = array2table(forkSVDEatData,'VariableNames',columnTitles);
% T = [array2table(users) T];
% %Writes out data for graphs to be made in csv
% writetable(T,strcat(outputPath,'\Fork_Eat_SVD_Data.csv'))

% % Fork SVD not eating data
% T = array2table(forkSVDNotEatData,'VariableNames',columnTitles);
% T = [array2table(users) T];
% % Writes out data for graphs to be made in csv
% writetable(T,strcat(outputPath,'\Fork_NotEat_SVD_Data.csv'))


% Saves figure for orientation data on every user

% x_axis = categorical(users);
% y_axis = forkSVDEatData(1:30,1:4);
...
% x_axis = categorical(users);
% y_axis = forkSVDEatData(1:30,5:7);
