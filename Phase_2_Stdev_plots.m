...
dataPath = strcat(scriptPath,'\Phase_2_Data\Stdev');
%Path to Myo Data
myoPath = strcat(scriptPath,'\MyoData');
% Path where the graph files will be written out
outputPath = strcat(scriptPath,'\Phase_2_Data\Graphs\Stdev');
...
for i = 1 : size(userNums, 1)
    ...
    forkStdevEatData = [forkStdevEatData;Fork_Eat_Data];

    userForkEat = userData(contains(userData,'Fork_NotEat'));
    load(strcat(dataPath,'\',userForkEat{1}));
    forkStdevNotEatData = [forkStdevNotEatData;Fork_NotEat_Data];
...
save(fullfile(phase3DataPathEat, strcat('\Fork_Stdev_Eat.mat')),'forkStdevEatData');
save(fullfile(phase3DataPathNotEat, strcat('\Fork_Stdev_NotEat.mat')),'forkStdevNotEatData');
...
for i = 1 : size(forkStdevEatData, 1)
    ori_z_eat = forkStdevEatData(:,i);
    ori_z_not_eat = forkStdevNotEatData(:,i);
    fig = plot(users,ori_z_eat,'r-');
    hold on;
    fig = plot(users,ori_z_not_eat,'b-');
    title(strcat(upper(columnTitles{i}), " STDEV"));
    legend(labels);
    legend('Location', 'eastoutside');
    set(gcf, 'Units', 'Normalized', 'OuterPosition', [0, 0.04, 1, 0.96]);
    hold off;
    savepath = strcat(outputPath,"\",columnTitles{i},"_stdev.png");
    saveas(fig,savepath,'png'); 
end

% Fork min eating data 
% T = array2table(forkStdevEatData,'VariableNames',columnTitles);
% T = [array2table(users) T];
% %Writes out data for graphs to be made in csv
% writetable(T,strcat(outputPath,'\Fork_Eat_Stdev_Data.csv'))

% % Fork min not eating data
% T = array2table(forkStdevNotEatData,'VariableNames',columnTitles);
% T = [array2table(users) T];
% % Writes out data for graphs to be made in csv
% writetable(T,strcat(outputPath,'\Fork_NotEat_Stdev_Data.csv'))


% Saves figure for orientation data on every user

% x_axis = categorical(users);
% y_axis = forkStdevEatData(1:30,1:4);
...
% x_axis = categorical(users);
% y_axis = forkStdevEatData(1:30,5:7);
